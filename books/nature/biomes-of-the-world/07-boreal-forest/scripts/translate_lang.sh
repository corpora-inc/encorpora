...
**After writing**: run
\`\`\`
python3 -c "import json; d=json.load(open('$OUT')); assert len(d['segments'])==165; assert all((s.get('text') or '').strip() for s in d['segments']); print('OK', len(d['segments']))"
\`\`\`
and paste its output as the last line of your response. If the assertion fails, fix and retry.

Do not open any other file. Do not modify the source. Report only "DONE: 165 segments" or the specific failure.
PROMPT_EOF
...
assert len(d['segments'])==165, f'expected 165 got {len(d[\"segments\"])}'
assert all((s.get('text') or '').strip() for s in d['segments']), 'empty text'
print('  OK: 165 segments, all text non-empty')
"
