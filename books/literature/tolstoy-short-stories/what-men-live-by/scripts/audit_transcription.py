...
def load_allowance(lang: str) -> dict[str, str]:
...
def classify_plausibility(rec: dict, rate: dict | None) -> str:
    ...
    Tightening rule (added 2026-05-03 after ch05-196 escape on IT 0.2.0):
    A segment whose audio is well under p50 AND whose heard text is meaningfully
    shorter than expected is HEARD_TEXT_FITS_BETTER even if d/e is technically
    in the [p5, p95] band. The two-condition guard keeps Whisper transliteration
    artifacts (heard_chars ≈ expected_chars) from tripping the rule.
    """
    if rate is None or rec.get('audio_duration_sec') is None:
        return 'AMBIGUOUS'
    de = rec.get('sec_per_expected_char')
    dh = rec.get('sec_per_heard_char')
    p5, p50, p95 = rate['p5'], rate['p50'], rate['p95']
    exp_chars = rec.get('expected_chars') or 0
    heard_chars = rec.get('heard_chars') or 0
    if de is None:
        return 'AMBIGUOUS'
    if de < p5 * 0.75:
        return 'OFF_SCRIPT_OR_TRUNCATED'
    # Strong signal: rate-deficit AND heard-text shorter ⇒ tail-drop.
    if (de < 0.92 * p50
            and exp_chars > 0
            and heard_chars < 0.92 * exp_chars):
        return 'HEARD_TEXT_FITS_BETTER'
    # Original signal: d/e below band, d/h in band.
...
def main() -> int:
    ...
    args = ap.parse_args()
    LANG = args.lang
    CALIB_DIR = Path(f'/home/skyl/projects/ttsctl/calibration/{BOOK_SLUG}_{VOICE_ID}')
    ...
    allowance = load_allowance(LANG)
    print(f'allowance entries: {len(allowance)}', flush=True)
