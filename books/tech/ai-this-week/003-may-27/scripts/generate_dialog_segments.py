...
# Local phonetics module — display → spelled-out for tts.text (Ep 2+).
sys.path.insert(0, str(Path(__file__).resolve().parent))
from phonetics import spell_out  # noqa: E402
...
def parse_dialog(filepaths: list[str], book_id: str) -> list[dict]:
    ...
    for filepath in filepaths:
        ...
        for block in blocks:
            ...
            # Spell out numerals / model versions / Roman numerals etc.
            # for the TTS path while leaving the display field as the
            # natural numeric form.
            tts_text = spell_out(content)
