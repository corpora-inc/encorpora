#!/usr/bin/env bash
# AI This Week Ep3 — per-language pipeline: translate → gate → generate →
# fixup → master → audit → gate → publish → patch-catalog → verify → record.
# Auto-ship: publishes only if both gates pass; otherwise DROPS the language
# (no flawed audio ships). Logs to /tmp/ep3_<lang>/.
#
...
LOG="/tmp/ep3_${LANG_CODE}"; mkdir -p "$LOG"
...
case " ta te gu kn ur th ja bn mr ne " in
...
record() { # status note
  echo "{\"issue\":3,\"lang\":\"$LANG_CODE\",\"date\":\"$(stamp)\",\"version\":\"$VERSION\",\"status\":\"$1\",\"provider\":\"${PROVIDER[1]}\",\"note\":\"$2\"}" >> "$REC"
}
...
# 2. Translation gate (no spend if broken)
...
ttsctl generate "$PACK" --lang "$LANG_CODE" --device cuda > "$LOG/gen.log" 2>&1
GEN=$?
echo "[$LANG_CODE] generate exit=$GEN"

# 3b. Recover transient FAILED segments (e.g. Gemini empty_response on short
#     reactions) with jittered retry passes before giving up. Gemini empties
#     are stochastic; a few fresh passes usually clear them.
...
# 8. Publish
echo "[$LANG_CODE] publishing v$VERSION…"
ttsctl publish "$PACK" --lang "$LANG_CODE" --voice-id gemini-vindy --tier public --version "$VERSION" > "$LOG/publish.log" 2>&1
...
ok=any(n.get('bookId')=='book_ai_this_week_2026_05_27' and n.get('language')=='$LANG_CODE' for n in d['narrations'])
