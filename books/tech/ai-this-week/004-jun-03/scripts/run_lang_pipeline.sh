...
# gates pass; otherwise DROPS the language. Logs to /tmp/ep4_<lang>/.
#
...
LOG="/tmp/ep4_${LANG_CODE}"; mkdir -p "$LOG"
...
record() { # status note
  echo "{\"issue\":4,\"lang\":\"$LANG_CODE\",\"date\":\"$(stamp)\",\"version\":\"$VERSION\",\"status\":\"$1\",\"provider\":\"${PROVIDER[1]}\",\"note\":\"$2\"}" >> "$REC"
}
...
ok=any(n.get('bookId')=='book_ai_this_week_2026_06_03' and n.get('language')=='$LANG_CODE' for n in d['narrations'])
