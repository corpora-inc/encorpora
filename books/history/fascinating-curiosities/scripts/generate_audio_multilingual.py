#!/usr/bin/env python3
"""
Multilingual audio generation pipeline for Fascinating Curiosities book series.

Uses ChatterboxMultilingualTTS for non-English narration with voice cloning.
Fork of generate_audio.py — same pipeline (TTS → Align → Encode) but using
the multilingual model with language_id parameter.
...
Usage:
    python generate_audio_multilingual.py \
        ...
        --cfg-weight 0.8 --format opus --device cuda
...
Key differences from generate_audio.py (English-only):
    - Uses ChatterboxMultilingualTTS instead of ChatterboxTTS
    - Adds --language-id arg passed to model.generate(language_id=...)
    - Default repetition_penalty=2.0 (vs 1.2 for English model)
    - Uses model.sr for sample rate instead of hardcoded 24000
    - Patches Llama config to use eager attention (SDPA incompatible with
      output_attentions=True needed by AlignmentStreamAnalyzer)
"""
...
def generate_tts(
    ...
    """
    Generate speech audio from text using Chatterbox Multilingual TTS.
...
def process_segment(
    ...
    audio, sample_rate = generate_tts(
        tts_text, voice_path, tts_model,
        language_id=language_id, device=device,
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate multilingual narrated audio for book segments"
    )
    parser.add_argument(
        "--segments", required=True,
        help="Path to segments JSON (e.g. segments_es.json)"
    )
    parser.add_argument(
        "--voice", required=True,
    ...
    parser.add_argument(
        "--language", default="es",
        help="Language code for alignment and manifest (default: es)"
    )
    parser.add_argument(
        "--language-id", default="es",
        help="Language ID for Chatterbox multilingual model (default: es)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--cfg-weight", type=float, default=0.8,
        help="Chatterbox CFG weight parameter (default: 0.8)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--repetition-penalty", type=float, default=2.0,
        help="Repetition penalty (default: 2.0 for multilingual model)"
    )
    ...
    # Load models
    print(f"\nLoading Chatterbox Multilingual TTS on {args.device}...")
    t0 = time.time()
    from chatterbox.mtl_tts import ChatterboxMultilingualTTS
    tts_model = ChatterboxMultilingualTTS.from_pretrained(device=args.device)
    print(f"Chatterbox Multilingual loaded in {time.time() - t0:.1f}s")

    # Update manifest sample rate from model
    manifest["sample_rate"] = tts_model.sr
    ...
    # Process segments
    print(f"\n{'='*60}")
    print(f"Starting multilingual audio generation pipeline")
    print(f"  Language: {args.language} (language_id: {args.language_id})")
    print(f"{'='*60}\n")
    ...
    elapsed = time.time() - t_start
    print(f"\n{'='*60}")
    print(f"Multilingual pipeline complete!")
