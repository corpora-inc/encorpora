...
PAPER="${PAPER:-6x9}"
GEOMETRY_HEADER=""
usage() { echo "Usage: ./build.sh [--paper=6x9|letter|a4] [--focus=NUM|FILE] [--window=N] [--list] [--pdf-only|--epub-only]"; }
while [[ $# -gt 0 ]]; do
  case "$1" in
    --paper=*) PAPER="${1#*=}"; shift ;; --paper) PAPER="$2"; shift 2 ;;
    --focus=*) FOCUS="${1#*=}"; shift ;; --focus) FOCUS="$2"; shift 2 ;;
    --window=*) WINDOW="${1#*=}"; shift ;; --window) WINDOW="$2"; shift 2 ;;
    --list) LIST=true; shift ;; --pdf-only) BUILD_EPUB=false; shift ;; --epub-only) BUILD_PDF=false; shift ;;
    *) echo "Unknown argument: $1"; usage; exit 1 ;;
  esac
done
case "$PAPER" in 6x9) ;; letter|full) GEOMETRY_HEADER="custom_geometry_letter.tex" ;; a4) GEOMETRY_HEADER="custom_geometry_a4.tex" ;; *) echo "Unknown paper size: $PAPER"; exit 1 ;; esac
shopt -s nullglob
ALL_FILES=( $INPUT_GLOB )
if [[ ${#ALL_FILES[@]} -eq 0 ]]; then echo "No manuscript files found"; exit 1; fi
...
if [[ -n "$FOCUS" ]]; then
  ...
  if [[ ! "$WINDOW" =~ ^[0-9]+$ ]]; then echo "Window must be a non-negative integer"; exit 1; fi
  idx=-1
  ...
  if (( start < 0 )); then start=0; fi; if (( end >= ${#ALL_FILES[@]} )); then end=$(( ${#ALL_FILES[@]} - 1 )); fi
  count=$(( end - start + 1 ))
