...
class AdmobPlugin: Plugin {
    ...
    @objc func showBanner(_ invoke: Invoke) throws {
        let args = try invoke.parseArgs(BannerAdArgs.self)

        DispatchQueue.main.async { [weak self] in
            guard let self = self else {
                invoke.resolve(["shown": false, "error": "plugin deallocated"])
                return
            }
            ...
            guard let rootVC = self.getRootViewController() else {
                invoke.resolve(["shown": false, "error": "no root view controller"])
                return
            }

            let unitId = args.adUnitId ?? self.testBannerId
            let position = args.position ?? "bottom"

            let adSize: GADAdSize
            switch args.size ?? "banner" {
            case "largeBanner": adSize = GADAdSizeLargeBanner
            case "mediumRectangle": adSize = GADAdSizeMediumRectangle
            case "fullBanner": adSize = GADAdSizeFullBanner
            case "leaderboard": adSize = GADAdSizeLeaderboard
            default: adSize = GADAdSizeBanner
            }
            ...
            print("[ADMOB] Banner shown at \(position)")
            invoke.resolve(["shown": true])
        }
    }

    @objc func hideBanner(_ invoke: Invoke) throws {
        DispatchQueue.main.async { [weak self] in
            self?.bannerView?.removeFromSuperview()
            self?.bannerView = nil
            print("[ADMOB] Banner hidden")
