...
const scheduleManifestUpdate = () => {
  ...
  updateTimer = setTimeout(async () => {
    try {
      ...
      console.error("[hover-runner] Failed to update dev manifest:", err)
    }
  }, 150)
}
...
const watchDist = () => {
  try {
    ...
    console.warn("[hover-runner] Dist watcher unavailable:", err)
  }
}
...
const run = (cmd, args, cwd, name) => {
  ...
  child.on("exit", (code) => {
    if (code && code !== 0) {
      console.error(`[hover-runner] ${name} exited with ${code}`)
    }
    process.exit(code ?? 0)
  })
  return child
