...
fn main() {
    ...
    if matches!(target_os, TargetOs::Android) {
        ...
        match android_abi {
            "arm64-v8a" => {
                // CORPAN FORK: upstream hardcodes the conservative `armv8-a`
                // baseline here, which leaves ggml's vectorized Q4_K matmul
                // kernels (arch/arm/quants.c, gated on __ARM_FEATURE_DOTPROD)
                // compiled OUT → scalar fallback → ~20 tok/s prefill on a
                // flagship. Use ggml's own arch knob instead (mirrors the
                // crate's Linux/aarch64 path below) so the dotprod + fp16
                // kernels are built in. dotprod is ARMv8.2 — effectively
                // universal on arm64 Android since ~2018 (our minSdk-26 floor).
                // We deliberately do NOT add +i8mm/+sme here: those are
                // ARMv8.6/ARMv9.2 (2021+ only) and baking them into the static
                // baseline would SIGILL on older devices. (i8mm for Q4_K would
                // need GGML_CPU_ALL_VARIANTS or a Q4_0 model — see
                // tauri-plugin-corpan-llm/ANDROID_PERF.md.)
                config.define("GGML_CPU_ARM_ARCH", "armv8.2-a+dotprod+fp16");
            }
            "armeabi-v7a" => {
