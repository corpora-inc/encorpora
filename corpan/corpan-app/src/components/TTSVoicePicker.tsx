// encorpora/corpan/corpan-app/src/components/TTSVoicePicker.tsx
//
// The voice-picker "meat" — the phase machine (Android health probe +
// auto-recover), the live voices list, the rescue cards, the calm
// confident-default rows, and the power-user per-language grid. Rendered
// as a plain fragment (title + intro + body) so it can be hosted in BOTH:
//
//  - onboarding (OnboardingTTSInstructions wraps it in OnboardingShell with
//    a pinned Continue/Back), and
//  - settings (TTSSettingsDrawer hosts it in a bottom drawer so the user can
//    re-tune voices in place without re-walking onboarding).
//
// All voice selections persist live to the settings store, so neither host
// needs a "save" step — closing the surface keeps the choices.
...
const SAMPLES: Record<string, string> = {
    ...
    fa: "مشتاقانه منتظر یادگیری با شما هستم.",
    th: "ฉันตั้งตารอที่จะได้เรียนรู้กับคุณ",
    id: "Saya menantikan untuk belajar bersama Anda.",
    jv: "Aku ngarep-arep bisa sinau bareng karo sampeyan.",
    su: "Abdi ngarep-ngarep tiasa diajar babarengan sareng anjeun.",
    ms: "Saya tidak sabar untuk belajar bersama anda.",
    tl: "Inaasahan kong matuto kasama ka.",
    sw: "Ninatazamia kwa hamu kujifunza pamoja nawe.",
    no: "Jeg gleder meg til å lære sammen med deg.",
    sv: "Jag ser fram emot att lära mig tillsammans med dig.",
    da: "Jeg glæder mig til at lære sammen med dig.",
    fi: "Odotan innolla, että saan oppia kanssasi.",
    el: "Ανυπομονώ να μάθω μαζί σου.",
    he: "אני מצפה ללמוד יחד איתך.",
...
export type TTSVoicePickerProps = {
    /**
     * Called when the user dismisses a rescue card via "Skip". In onboarding
     * this advances to the next step; in the settings drawer it closes the
     * drawer. Voice selections persist live regardless, so this is dismissal
     * only — never a "save".
     */
    onSkip?: () => void;
};
...
/**
 * The shared voice-picker body. Renders a heading, an intro line, and the
 * phase-driven content. No wrapper / footer of its own — the host (onboarding
 * shell or settings drawer) supplies the chrome.
 */
export function TTSVoicePicker({ onSkip }: TTSVoicePickerProps = {}) {
    ...
    async function refreshVoices(): Promise<boolean> {
        ...
        try {
            ...
            console.warn("[ttsVoicePicker] refreshVoices: getVoices failed", e);
        }
        if (os === "android") {
            try {
                ...
                console.warn("[ttsVoicePicker] refreshVoices: engine status failed", e);
            } finally {
    ...
    async function runDiagnose() {
        ...
        try {
            ...
            try {
                ...
                console.warn("[ttsVoicePicker] probe failed", e);
            ...
            try {
                ...
                console.warn("[ttsVoicePicker] auto-recover failed", e);
            }
            ...
            try {
                ...
                console.warn("[ttsVoicePicker] re-probe failed", e);
            }
        ...
        const interval = window.setInterval(async () => {
            ...
            try {
                ...
                if (voicesSignature(list) !== voicesSignature(voicesRef.current)) {
                    console.info(
                        "[ttsVoicePicker] voice set changed on poll — refreshing list",
                    );
                    setVoices(list);
                }
            } catch (e) {
                console.warn("[ttsVoicePicker] voice poll failed", e);
            } finally {
    ...
    function handleSkip() {
        // Engine-driven: dismiss the rescue card. Onboarding advances to the
        // next graph node; the settings drawer closes. Legacy fallback advances
        // to the old wizard's Finish step (5).
        ;(onSkip ?? (() => setStep(5)))()
    }
