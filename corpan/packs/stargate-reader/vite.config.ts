...
// constant (`__STARGATE_READER_VERSION__`) injected via `define` below — and
...
const stargateManifest = JSON.parse(
  ...
  return {
    ...
    closeBundle() {
      ...
      try {
        ...
        console.log(`[stargate-reader] Updated manifest devRevision: ${manifest.devRevision}`)
      } catch (err) {
        console.error("[stargate-reader] Failed to update manifest:", err)
      }
    },
  }
}
...
export default defineConfig({
  ...
  define: {
    "process.env": {},
    __STARGATE_READER_VERSION__: JSON.stringify(stargateManifest.version),
  },
  resolve: {
    alias: {
