/**
 * beatlounge — the GROOVES IMMERSIVE view: a browsable bank of world rhythms
 * grouped by family, with a selected-rhythm detail and Apply / Vary / Evolve /
 * Randomize actions that operate on the drum track (and, when phrases are on,
 * lay saved snippets onto the groove).
 *
 ...
 *   • ACTIONS — Apply (write it), Vary (small change), Evolve (drift), Randomize
 *     (re-roll). Each dispatches ONE undo batch via the registry actions.
 *   • OPTIONS — an Intensity slider + a "Lay phrases on the groove" toggle (only
 *     active when a phrase track + bank exist).
 *
...
export const GroovesImmersive = ({ store, host }: Props) => {
  ...
  const runGroove = (
    ...
    if (result.commands.length === 0) {
      console.warn("[beatlounge/grooves] action produced no commands:", action.name, result.summary)
    ...
    <div className="bl-grooves">
      ...
      <div className="bl-grooves-toolbar" data-bl-nocapture>
        <div className="bl-grooves-title">
          <span className="bl-grooves-title-mark"><GrooveMark size={18} /></span>
          Grooves
        </div>
        <input
          type="search"
        ...
        <aside className="bl-grooves-detail" aria-live="polite">
          ...
          <div className="bl-grooves-options" data-bl-nocapture>
            ...
            <label
              ...
              title={
                phrasesPossible
                  ? "Lay saved phrase snippets onto the groove's onsets"
                  : "Save phrases and add a phrase track to enable"
              }
            >
              <input
                type="checkbox"
