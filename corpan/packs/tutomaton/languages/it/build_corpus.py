"""Build Italian sqlite corpus for Tutomaton.
...
Inputs:
  - ~/data/kaikki/kaikki-it.jsonl     (download from kaikki.org;
                                                  cached, gitignored)
...
LANG_CODE = "it"      # ISO 639-1, e.g. "en", "es", "zh", "fr"
LANG_NAME = "Italian"      # human name, e.g. "English", "Spanish"
...
DB_PATH = HERE / "data" / f"it.sqlite3"
KAIKKI_CACHE = Path.home() / "data" / "kaikki" / f"kaikki-it.jsonl"
...
def iter_kaikki(path: Path):
    ...
    if not path.exists():
        ...
        print(f"      Download from https://kaikki.org/dictionary/Italian/", file=sys.stderr)
        return
    with path.open() as f:
        for line in f:
            line = line.strip()
...
def main():
    print(f"Building Italian (it) corpus → {DB_PATH}")
    conn = init_db()
    populate_words(conn)
