...
const VOCAB_PATTERNS: RegExp[] = [
  /\bhow\s+do\s+you\s+say\s+["'']?(.+?)["'']?\s+in\s+French\b/i,
  /\bwhat['']s?\s+the\s+French\s+(?:word|term)\s+for\s+["'']?(.+?)["'']?[?.]?$/i,
  /\btranslate\s+["'']?(.+?)["'']?(?:\s+to\s+French)?[?.]?$/i,
  ...
  for (const [pat, topic] of DIRECT_TOPIC_MAP) {
    if (pat.test(msg)) {
      ...
      if (lesson) {
        ...
        if (lesson.l1_notes_json && l1Code !== "fr") {
          try {
