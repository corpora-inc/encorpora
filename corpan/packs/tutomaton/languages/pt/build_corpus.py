"""Build Portuguese sqlite corpus for Tutomaton.
...
Inputs:
  - ~/data/kaikki/kaikki-pt.jsonl     (download from kaikki.org;
                                                  cached, gitignored)
...
LANG_CODE = "pt"      # ISO 639-1, e.g. "en", "es", "zh", "fr"
LANG_NAME = "Portuguese"      # human name, e.g. "English", "Spanish"
...
DB_PATH = HERE / "data" / f"pt.sqlite3"
KAIKKI_CACHE = Path.home() / "data" / "kaikki" / f"kaikki-pt.jsonl"
...
def iter_kaikki(path: Path):
    ...
    if not path.exists():
        ...
        print(f"      Download from https://kaikki.org/dictionary/Portuguese/", file=sys.stderr)
        return
    with path.open() as f:
        for line in f:
            line = line.strip()
...
def main():
    print(f"Building Portuguese (pt) corpus → {DB_PATH}")
    conn = init_db()
    populate_words(conn)
