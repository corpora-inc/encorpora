...
const scheduleManifestUpdate = () => {
  ...
  updateTimer = setTimeout(async () => {
    try {
      ...
      console.error("[pronunciation-coach] Failed to update dev manifest:", err)
    }
  }, 150)
}
...
const watchDist = () => {
  try {
    ...
    console.warn("[pronunciation-coach] Dist watcher unavailable:", err)
  }
}
...
const run = (cmd, args, cwd, name) => {
  ...
  child.on("exit", (code) => {
    if (code && code !== 0) {
      console.error(`[pronunciation-coach] ${name} exited with ${code}`)
    }
    process.exit(code ?? 0)
  })
  return child
