#!/usr/bin/env python3
"""Generate Teletron chrome translations and inject them into src/i18n.ts.
...
Teletron's manifest.json has no `languages[]` array, so the target locales are
derived from the keys of `displayName` (minus en).

Usage (from packs/teletron):
    OPENAI_API_KEY=... python3 tools/gen_i18n.py            # all manifest langs
    OPENAI_API_KEY=... python3 tools/gen_i18n.py fr de ja   # just these
    python3 tools/gen_i18n.py --from-json <file.json>       # offline inject
...
MODEL = os.environ.get("TELE_I18N_MODEL", "gpt-4o-mini")
...
def target_langs(argv):
    ...
    # Teletron has no `languages[]`; derive targets from displayName keys.
    return [code for code in m["displayName"].keys() if code != "en"]
...
def sys_prompt(lang):
    ...
    return (
        "You are a professional UI localizer for a friendly, calm penpal app called Teletron. "
        ...
        "for example {name}, {count}, {size}, {percent}, {status}, {outcome} must appear unchanged. "
        "Keep it warm, plain and reassuring (this is a 'safe penpals' product); avoid AI jargon. "
