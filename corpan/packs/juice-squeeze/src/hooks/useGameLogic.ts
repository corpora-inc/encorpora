...
export function useGameLogic(hostApi: HostApi) {
  ...
  const loadNext = useCallback(async () => {
    ...
    try {
      ...
      console.error("[juice-squeeze] load error", err)
  ...
  const runWin = useCallback(() => {
    ...
    console.log("[juice-squeeze] bottle complete", {
  ...
  const onSentenceChanged = useCallback(() => {
    ...
    console.log("[juice-squeeze] placement", {
      bank: store.bankOrder.length,
      placed: words.length,
      won,
    })
    if (won) {
      console.log("[juice-squeeze] win detected")
      runWin()
    } else {
      updateDebug()
    }
