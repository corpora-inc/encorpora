...
class LiquidControllerImpl implements LiquidController {
  ...
  mount(canvasParent: HTMLElement): void {
    ...
    this.disposed = false
    if (!canRenderLiquid()) {
      console.log("[juice-squeeze] liquid mount skipped (no 2d canvas)")
      return
    }
    ...
    void import("./LiquidStage")
      .then(({ createLiquidStage }) => {
        ...
        console.log("[juice-squeeze] liquid mounted")
      })
      .catch((err) => {
        console.warn("[juice-squeeze] liquid stage failed to load", err)
      })
  }
  ...
  setFill(level01: number, opts?: { animate?: boolean }): void {
    ...
    console.log("[juice-squeeze] liquid setFill", { level: Number(lvl.toFixed(3)) })
    this.stage?.setFill(lvl, opts)
  }

  triggerWin(): void {
    console.log("[juice-squeeze] liquid triggerWin")
    this.stage?.triggerWin()
  }

  triggerBottleComplete(): void {
    console.log("[juice-squeeze] liquid bottleComplete")
