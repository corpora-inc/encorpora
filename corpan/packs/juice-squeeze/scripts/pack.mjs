#!/usr/bin/env node
/**
 * Package juice-squeeze.zip — a code-only game pack (manifest.json + dist/).
 *
 ...
 * it to ~app-data/corpan-packs/juice_squeeze/ and serves it via corpan-pack://.
 *
 * Usage:
...
async function main() {
  ...
  const zipPath = path.join(packRoot, "juice-squeeze.zip")
  if (existsSync(zipPath)) {
    await rm(zipPath)
  }

  console.log("Creating juice-squeeze.zip...")
  execSync("zip -r juice-squeeze.zip manifest.json dist/", {
    cwd: packRoot,
    stdio: "inherit",
  })

  console.log("\nDone! juice-squeeze.zip is ready.")
}
