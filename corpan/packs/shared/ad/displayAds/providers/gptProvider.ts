...
export type GptProvider = {
  init(config: DisplayAdConfig, containers: Map<string, HTMLElement>, slotPrefix?: string): void
  refresh(): void
  dispose(): void
}
...
export const createGptProvider = (onBeforeRefresh?: () => void): GptProvider => {
  ...
  const slotDivId = (slot: DisplayAdSlot, prefix: string) => `${prefix}-gpt-${slot.position}`

  const init = (config: DisplayAdConfig, containers: Map<string, HTMLElement>, slotPrefix = "ad") => {
    ...
    void loadScript().then(() => {
      ...
      googletag.cmd.push(() => {
        for (const slotConfig of config.slots) {
          const divId = slotDivId(slotConfig, slotPrefix)
          ...
          const adUnitPath = slotConfig.adUnitPath
            ?? `/${config.networkCode}/${slotPrefix}-${slotConfig.position}`
