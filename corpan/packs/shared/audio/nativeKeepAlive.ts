...
export function listenForRemoteCommands(handlers: {
  ...
  for (const [event, handler] of Object.entries(simpleEventMap)) {
    ...
    internals.invoke("plugin:audio-keepalive|register_listener", {
      ...
      console.error(`[native] FAILED to register listener for ${event}:`, err)
    })
  ...
  if (handlers.onSeek) {
    ...
    internals.invoke("plugin:audio-keepalive|register_listener", {
      ...
      console.error("[native] FAILED to register listener for audio-keepalive:seek:", err)
    })
  ...
  if (handlers.onInterruptionEnded) {
    ...
    internals.invoke("plugin:audio-keepalive|register_listener", {
      ...
      console.error("[native] FAILED to register listener for audio-keepalive:interruptionEnded:", err)
    })
