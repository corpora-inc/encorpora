...
// Unique counter to avoid DOM ID collisions on double-mount (React strict mode)
let instanceCounter = 0

/**
 * Patch HtmlMeshRenderer to handle null offsetParent in embedded pack contexts.
 * The library assumes offsetParent is always non-null, but inside Corpan's
 * pack host the container can have offsetParent === null during initial layout.
 */
const patchHtmlMeshRenderer = () => {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const proto = HtmlMeshRenderer.prototype as any
  if (proto._aw_patched) return
  proto._aw_patched = true

  const orig = proto._updateContainerPositionIfNeeded as () => void
  proto._updateContainerPositionIfNeeded = function (this: unknown) {
    try {
      orig.call(this)
    } catch {
      // offsetParent null — container not yet in layout; skip this frame
    }
  }
}
...
export const createBillboardSystem = (
  ...
  // Ensure container has an ID and relative positioning for HtmlMeshRenderer
  if (!container.id) container.id = "aw-container-" + Date.now()
  container.style.position = "relative"

  // Patch library for null-safety before instantiation
  patchHtmlMeshRenderer()

  // Use unique container ID per instance to avoid collisions on double-mount
  const containerId = `aw-css3d-${++instanceCounter}`

  // Initialize HtmlMesh renderer — attach to pack container, not document.body
