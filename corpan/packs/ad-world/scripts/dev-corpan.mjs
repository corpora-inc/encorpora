...
const scheduleManifestUpdate = () => {
  ...
  updateTimer = setTimeout(async () => {
    try {
      ...
      console.error("[ad-world] Failed to update dev manifest:", err)
    }
  }, 150)
}
...
const watchDist = () => {
  try {
    ...
    console.warn("[ad-world] Dist watcher unavailable:", err)
  }
}
...
const run = (cmd, args, cwd, name) => {
  ...
  child.on("exit", (code) => {
    if (code && code !== 0) {
      console.error(`[ad-world] ${name} exited with ${code}`)
    }
    process.exit(code ?? 0)
  })
  return child
