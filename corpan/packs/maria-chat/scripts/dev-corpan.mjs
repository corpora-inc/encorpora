...
const scheduleManifestUpdate = () => {
  ...
  updateTimer = setTimeout(async () => {
    try {
      ...
      console.error("[maria-chat] Failed to update dev manifest:", err)
    }
  }, 150)
}
...
const watchDist = () => {
  try {
    ...
    console.warn("[maria-chat] Dist watcher unavailable:", err)
  }
}
...
const run = (cmd, args, cwd, name) => {
  ...
  child.on("exit", (code) => {
    if (code && code !== 0) {
      console.error(`[maria-chat] ${name} exited with ${code}`)
    }
    process.exit(code ?? 0)
  })
  return child
