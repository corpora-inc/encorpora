...
```
world-plaza/
├── contracts/        # @world-plaza/contracts — Zod schemas + types (the spine)
│   └── src/
...
`@world-plaza/contracts` is the single source of truth for every boundary
...
```ts
import { RoomTopology, Scene, Quest, parseScene } from "@world-plaza/contracts"
```
