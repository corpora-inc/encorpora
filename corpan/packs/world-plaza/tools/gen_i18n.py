#!/usr/bin/env python3
"""Generate World Plaza chrome translations and inject them into src/i18n/strings.ts.

Reads the English `en` dict (the source of truth) out of src/i18n/strings.ts, asks
the OpenAI API to translate the VALUES into every target language (preserving the
...
"tutomaton-i18n-tooling"). Two deltas: the catalog uses QUOTED dotted keys
("welcome.title": "…") instead of bare identifiers, and the .env search walks up
one extra level to reach encorpora/.env.
...
Usage (from packs/world-plaza):
    ...
    python3 tools/gen_i18n.py --from-json <file.json>       # inject prebuilt JSON

Idempotent: re-running regenerates the block. English is always kept verbatim.
Pitfalls already burned-in: never unicode_escape TS values (mojibake); the locale
regex allows yue; post-merge diff should be insertions-only.
"""
import json, os, re, sys, pathlib, urllib.request

HERE = pathlib.Path(__file__).resolve().parent
PACK = HERE.parent
I18N = PACK / "src" / "i18n" / "strings.ts"
...
def extract_en():
    ...
    if not m:
        sys.exit("could not find `const en: Dict = {...}` in strings.ts")
    body = m.group(1)
