...
function readMap(): PoseMap {
  try {
    ...
    // keep only well-formed entries (a corrupt row can never strand the player).
    const out: PoseMap = {}
 ...
 * `pair` IF one exists AND it's on walkable ground, else `null` (the caller then
 * frames the player on the active objective — today's default first-visit start).
 *
 * Pure decision (the store + the collision predicate are injected) so the
 * resume-vs-default choice is unit-testable without the whole world. The walkability
 * guard matters: a city re-layout could leave an OLD saved pose inside new geometry —
 * we never drop the player into a wall; we fall back to the framed default instead.
 */
export function resolveResumeSpawn(
  ...
  if (saved && !blocked(saved.x, saved.z, 0.6)) return saved
  return null
}
