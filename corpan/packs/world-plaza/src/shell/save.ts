/**
 * save — the persistence seam for World Plaza (DESIGN + minimal impl).
 *
...
export type SaveSnapshotProvider = () => Omit<WorldPlazaSave, "v" | "savedAt">

export function loadSave(): WorldPlazaSave | null {
  try {
    const raw = localStorage.getItem(SAVE_KEY)
    if (!raw) return null
    const parsed = JSON.parse(raw) as WorldPlazaSave
...
export function writeSave(snapshot: SaveSnapshotProvider): void {
  try {
    const body = snapshot()
    const record: WorldPlazaSave = { v: SAVE_VERSION, savedAt: Date.now(), ...body }
