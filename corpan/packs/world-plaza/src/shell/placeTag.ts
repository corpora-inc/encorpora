...
export function mountPlaceTag(opts: PlaceTagOptions): PlaceTagHandle {
  ...
  // The pin glyph (shown phone-portrait icon-only; decorative elsewhere).
  const pin = document.createElement("span")
  pin.className = "wp-placetag-pin"
...
.wp-placetag {
  ...
  padding: 5px 11px;
  border-radius: 11px;
  /* A QUIETER paper chip than the capsule — a small luggage tag, low contrast. */
  background: rgba(239, 227, 205, 0.62);
  -webkit-backdrop-filter: blur(5px) saturate(1.02);
  backdrop-filter: blur(5px) saturate(1.02);
  box-shadow: 0 2px 9px rgba(58, 47, 37, 0.14), inset 0 1px 0 rgba(255, 255, 255, 0.45);
...
.wp-placetag-pin { font-size: 13px; line-height: 1; display: none; }
.wp-placetag-text {
...
@media (max-width: 540px) and (orientation: portrait) {
  ...
  .wp-placetag-pin { display: inline; }
