...
import { createChatMediator, composeOutbound, type ChatMediator } from "./mediatedChat"
...
export interface MultiplayerOptions {
  ...
  room?: string
  /** the local player's durable id (the game uses "player-local"). */
  playerId: string
...
export function initMultiplayer(opts: MultiplayerOptions): MultiplayerHandle {
  ...
  function openChat(partnerId: PlayerId, partnerName: string): void {
    ...
    const panel = openChatPanel(
      ...
      partnerName,
      (text) => {
        // Optimistic local echo; send the mediated input to the partner.
        chat?.panel.appendSelf(text)
        const input = composeOutbound({
          from: localPlayerId,
          to: partnerId,
          interactionId,
          text,
          sourceLanguage: opts.learnerPair.native as LanguageCode,
  ...
  async function onChat(input: MediatedChatInput): Promise<void> {
    ...
    try {
      artifact = await mediator.lessonify(input, opts.learnerPair.native as LanguageCode)
    } finally {
      stopBridging()
    }
    panel.appendPeer(artifact, (label) => {
      // Tapping a suggested reply sends it (it's in the partner's language).
      panel.appendSelf(label)
      const reply = composeOutbound({
        from: localPlayerId,
        to: input.from,
        interactionId: input.interactionId,
        text: label,
        sourceLanguage: opts.learnerPair.target as LanguageCode,
        targetLanguage: opts.learnerPair.native as LanguageCode,
        mode: "beginner",
      })
      proto?.sendChat(reply)
    })
  }
