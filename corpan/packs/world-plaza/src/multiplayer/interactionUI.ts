...
export function openChatPanel(
  ...
  return {
    ...
    appendPeer(artifact, onReply) {
      ...
      m.appendChild(main)
      if (artifact.learnerText && artifact.learnerText !== artifact.visibleText) {
        const orig = document.createElement("div")
        orig.className = "wp-mp-msg-orig"
        orig.textContent = `${t("mp.chat.original")}: ${artifact.learnerText}`
        if (artifact.transliteration) orig.textContent += ` (${artifact.transliteration})`
        m.appendChild(orig)
      }
