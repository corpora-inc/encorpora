...
/**
 * AI-mediated chat: players NEVER exchange raw text/audio. The sender's device
 * cleans → moderates → translates → "lessonifies" into a typed artifact; the
 * server validates + moderates + audits + routes; the recipient's device
 * re-presents it for THEIR level/quest. The recipient receives a
 * language-learning artifact, not necessarily the original message.
 */
