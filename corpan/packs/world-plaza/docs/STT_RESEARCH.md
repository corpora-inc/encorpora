# World Plaza — Native On-Device STT for SPEAK CHALLENGES
...
The speak tools (`packs/world-plaza/src/challenges/tools/sttTools.ts`,
...
(Whisper), and World Plaza users do **not** have a Whisper model installed by
default — that's a 75 MB–1.6 GB download tied to the *pronunciation-coach* pack
(`pronunciation-coach/src/modelRegistry.ts`), not to World Plaza. With no model
...
(`packs/world-plaza/src/challenges/host.ts`) exposes
...
NPC loop (`packs/world-plaza/src/npc/voiceInput.ts`, `KeyboardVoiceInput` floor
...
- `tauri-plugin-stt` is a tightly-tuned whisper.cpp **scorer** — per-word
  ...
  Keep it exactly as-is (pronunciation-coach + the World Plaza Whisper fallback).
...
- **Confidence signals differ from Whisper's.** Native STT gives text + a coarse
  ...
  from the pronunciation-coach Whisper score — fine, since World Plaza challenges
...
- In-repo: `MODEL_STRATEGY.md` (NPC-loop sibling decision),
  `packs/world-plaza/src/challenges/tools/sttTools.ts`,
  `packs/world-plaza/src/challenges/host.ts`,
  `packs/world-plaza/src/npc/voiceInput.ts`,
