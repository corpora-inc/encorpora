# World Plaza — CI/CD, Web Deploy & Cloud Infra to Production
...
>
> **Reuse-first principle.** World Plaza is a Corpán pack, not a new product. It
...
- `docs/RELEASE_ENGINEERING.md` (P3, parallel) — the pack `dist` build, two-zip
  preview/full split, `manifest.json`, and the `world-plaza` catalog entry this
...
World Plaza's jobs are `paths`-scoped to `packs/world-plaza/**` so they only run
...
│       └── action.yml
└── workflows/
    ├── world-plaza-ci.yml        # PR + push gate (typecheck, test, build, conformance, server)
    ├── world-plaza-pages.yml     # GH Pages demo (push to main + tag)
    └── world-plaza-release.yml   # tag → publish pack zip + catalog + deploy server
```

### 1.2 PR gate — `world-plaza-ci.yml`

Triggers: `pull_request` and `push` to non-`main` branches, both filtered to
`paths: ['packs/world-plaza/**']`. Concurrency-grouped per ref so a force-push
...
defaults:
  run:
    working-directory: packs/world-plaza
  ...
  pack:
    ...
    steps:
      ...
      - uses: actions/upload-artifact@v4             # hand dist/ to the pages + release jobs without rebuilding
        with: { name: wp-dist, path: packs/world-plaza/dist }
  ...
  contracts-conformance:
    ...
    steps:
      ...
      - uses: actions/upload-artifact@v4
        with: { name: conformance-report, path: packs/world-plaza/conformance.xml }

  server:
    runs-on: ubuntu-latest
    defaults: { run: { working-directory: packs/world-plaza/server } }
    steps:
      - uses: actions/checkout@v4
      - uses: ./.github/actions/setup-node-cache
      - run: npm ci
      - run: npm run typecheck                       # server tsc --noEmit (imports @world-plaza/contracts)
...
- **Caching.** The composite `setup-node-cache` keys the npm cache on
  `package-lock.json` hashes for both `packs/world-plaza` and
  `packs/world-plaza/server` (two lockfiles). Babylon + Playwright dev-deps make
...
### 1.3 Release — `world-plaza-release.yml`

Trigger: `push` of a tag matching `world-plaza-v*` (e.g. `world-plaza-v0.1.0`).
...
jobs:
  ...
  build:
    ...
    steps:
      - uses: actions/checkout@v4
      - uses: ./.github/actions/setup-node-cache
      - run: npm --prefix packs/world-plaza ci
      - run: npm --prefix packs/world-plaza run test:run        # re-gate at tag
      ...
      - uses: actions/upload-artifact@v4
        with: { name: wp-pack, path: packs/world-plaza/build/*.zip }
  ...
  deploy-server:
    ...
    steps:
      ...
      - name: Build & push server image
        run: |
          IMG=$ECR/corpan-world-plaza-server:${GITHUB_REF_NAME#world-plaza-v}
          docker build -t "$IMG" packs/world-plaza/server
          docker push "$IMG"
...
### 2.2 Build — `world-plaza-pages.yml`

Triggers: `push` to `main` filtered to `packs/world-plaza/**`, plus the release
...
```yaml
name: world-plaza-pages
on:
  push: { branches: [main], paths: ['packs/world-plaza/**'] }
  workflow_run:                     # also after a successful release
    workflows: [world-plaza-release]
    types: [completed]
  ...
  build:
    ...
    steps:
      ...
      - name: Build standalone demo
        run: npm --prefix packs/world-plaza run build
        env:
          VITE_BASE: /world-plaza/                       # project-page base path
      ...
      - name: Stage site (index.html + dist + bundled demo assets)
        run: node packs/world-plaza/scripts/stage-pages.mjs   # see §2.3
      - uses: actions/upload-pages-artifact@v3
        with: { path: packs/world-plaza/.pages }
  deploy:
    needs: build
    runs-on: ubuntu-latest
...
- `VITE_BASE` makes asset URLs work under the project-page sub-path
  `https://corpora-inc.github.io/<repo>/world-plaza/`. A custom domain
...
posture in `main.tf`:
- The `world-plaza` pack zip + catalog entry live under existing
...
- **Signed URLs for Plus-gated content:** reuse the existing
  ...
  asset kit or a full-experience zip, add a `pack-store/world-plaza/premium/*`
...
2. `world-plaza-ci.yml`: typecheck + vitest + build + **conformance gate** +
...
3. `world-plaza-release.yml` `build` + `publish-catalog` stages only (OIDC
   `corpan-ci-publisher`, pack zip → `pack-store/`, catalog upsert, CDN
   invalidate). Reuses the entire existing S3/CloudFront stack.
4. `world-plaza-pages.yml`: standalone demo on GitHub Pages (`VITE_DEMO=1`,
