# World Plaza — Realtime Multiplayer Presence (M1)

The north-star magic, first milestone: **two app windows = two real players in
the same plaza, seeing each other walk around in real time.** This document
covers the server, the client presence layer, how to run two clients, the exact
`game.ts` wiring, and the seam for AI-mediated chat next.

This milestone is **presence + movement ONLY**. Remote players appear as people
walking; making them talkable + mediated-chat is the next milestone (the seam is
documented below).
...
## 5. The seam for AI-mediated chat (next milestone)

This milestone is movement-only, but the architecture is built so mediated chat
slots in **without touching the movement hot-path**:

- **New room message, not a new transport.** Client:
  `room.send("chat", MediatedChatInput)` — each device first cleans + translates +
  "lessonifies" its line locally (the contract already defines
  `MediatedChatInput` / `MediatedChatArtifact`). **Never raw UGC.**
- **Server routes/moderates** in a new `onMessage("chat", …)` handler and emits,
  per recipient, a `MediatedChatArtifact` framed by THEIR quest.
- **Client surfaces it on the relevant `RemoteAvatar`** (which already knows that
  player's id + live position). The avatar's animator already exposes a
  `talk()`/mouth channel, so a mediated line animates the speaker with zero new
  rendering work.

Nothing in the presence/movement path changes for that — it's purely additive.
...
barrier into the lesson. The sender's line is wrapped into a typed
`MediatedChatInput` and routed; the RECIPIENT's device asks its local model to
`lessonify` it into a `MediatedChatArtifact`: a natural translation into the
reader's language, the original + transliteration, a short gloss, 2–3 tappable
suggested replies (in the PARTNER's language, so replying is practice), and one
tiny lesson note. It reuses the shared NPC `ModelBroker` (honouring the single
in-process model slot — LLM vs Whisper exclusivity), and DEGRADES to a clean
passthrough (original text shown as-is) whenever the model is unavailable — chat
never breaks, the barrier is just un-bridged that turn (logged, not silent). The
model is also asked to refuse contact-info/links/coercion (`blocked` → softened).
