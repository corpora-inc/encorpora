# World Plaza — DGX Spark Asset Generation Brief

Ready-to-run directions for an agent running on the **DGX Spark** to generate the
3D assets that the World Plaza **Look layer** will consume. Hand this whole file
...
> You are an asset-generation agent on a DGX Spark for "World Plaza". Read this
...
When this batch lands, the World Plaza `create3DLook()` (a new `WorldLook`
