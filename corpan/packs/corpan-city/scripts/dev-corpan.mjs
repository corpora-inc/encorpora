 ...
 * corpan_city from http://localhost:8989/corpan-city/manifest.json.
...
const scheduleManifestUpdate = () => {
  ...
  updateTimer = setTimeout(async () => {
    try {
      ...
      console.error("[corpan-city] Failed to update dev manifest:", err)
    }
  }, 150)
}
...
const watchDist = () => {
  try {
    ...
    console.warn("[corpan-city] Dist watcher unavailable:", err)
  }
}
...
const run = (cmd, args, cwd, name) => {
  ...
  child.on("exit", (code) => {
    if (code && code !== 0) console.error(`[corpan-city] ${name} exited with ${code}`)
    process.exit(code ?? 0)
  })
  return child
}
