# Game-Dev Playbook — hard-won lessons from Corpan City

> Read this FIRST if you are an agent building a new game pack (or extending this
> one). Corpan City is the furthest Corpán game development has gotten — a premium,
...
`@corpan-city/contracts` (Zod schemas) + `src/contracts/runtime.ts` (runtime
...
run immediately. Corpan City's seams (copy the pattern, not the names):
...
Corpan City's whole design is three ORTHOGONAL axes so different agents own different
...
In Corpan City the river was painted as blue ground but had no collider. Fix
...
## Appendix — the Corpan City file map (where each facet lives)
...
- Contracts: `@corpan-city/contracts` + `src/contracts/runtime.ts`.
