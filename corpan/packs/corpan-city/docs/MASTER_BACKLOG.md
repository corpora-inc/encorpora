# Corpan City — MASTER BACKLOG (the must-solve-before-first-ship list)
...
| # | Item | Status | Notes |
|---|------|--------|-------|
| A1 | **Stack/language reactivity** — corpan-city must FULLY derive from the Corpán stack (target/native langs). Currently `learnerPair` is hardcoded `es/en` from the quest JSON (the bug: switched stack to EN-from-ES, world still EN→ES). Multi-target stack → premium **language-chooser interlude**. Exit → flip stack → return → whole game reflects new stack. Premium **welcome/onboarding** into corpan-city. Single-language stack must work (SINGLE_LANGUAGE_RULE). | **IN-FLIGHT** | Agent: *World Entry / Stack Reactivity*. The front door — everything depends on it. |
