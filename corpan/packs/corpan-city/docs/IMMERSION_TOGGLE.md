# Corpan City — Per-Track Total Immersion Toggle
...
`DialogueUIStrings` (`inputPlaceholder`, `close`, `replay`, `challengeLabel`,
`playOffer`), the NPC `subtitle`, and the suggested-reply **chips**. The
microphone is intentionally icon-only, so its recording states add no localized
chrome.
