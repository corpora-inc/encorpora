# Corpan City — Production Multiplayer (P1)
...
3. **Reliability** — reconnection exists but with no presence-token resume or TTL
   sweep. Economy/trade anti-cheat is contracted but unimplemented server-side.
4. **Mediated-chat defense in depth** — two local model passes, trusted server
   routing, and rate limits exist. A future server-side classifier/audit layer
   can add another independent safety boundary.
...
### 4.1 The posture: every message gets two independent local safety passes

The sender's device cleans raw text into a safe intent before it enters a typed
`MediatedChatInput`. The server validates, rate-limits, stamps trusted routing
identity, and forwards that intent. The recipient treats it as untrusted, cleans
it again, translates it into their learning language, and renders only a typed
`MediatedChatArtifact`. Either local model failing produces a fixed harmless
fallback rather than exposing unreviewed text.
...
PlazaRoom.onMessage("chat"):
  ...
  3. AUTHZ: ignore caller-supplied `from`; stamp the sender from the live session.
            Resolve `to` only through a CURRENT co-occupant of THIS room.
  4. ROUTE: stamp the recipient's current target language and forward the typed
     safe intent to that recipient only.
   ▼
client B ← room.send("chat-deliver", MediatedChatInput)
   │
   └─ recipient local model independently cleans + translates
      → MediatedChatArtifact { visibleText, naturalTranslation?,
          transliteration?, gloss?, suggestedReplies[], lessonNotes[],
          moderation, safetyClass }
```

The server never trusts caller-supplied routing identity. The recipient UI never
renders the delivered intent directly; only the second local model's validated
artifact or a fixed harmless fallback can be displayed.

### 4.3 Why this is age-3+ safe

- **Two independent local safety passes.** Sender cleaning happens before the
  wire; recipient cleaning and translation happens before display.
- **Consent-gated:** no messages without a mutually accepted `chatOpen`.
- **PII/link checks are deterministic after both model passes.** A failed or
  invalid pass produces a fixed harmless fallback.
...
Implement the §4 sender-clean → trusted-route → recipient-clean/translate flow.
Suggested-reply rails for beginners. Surface the line on the `RemoteAvatar`'s
existing `talk()` channel.
- **Exit:** two players chat; the recipient receives a translated learning
  artifact, deterministic PII/link checks pass, and every failure path displays
  only a fixed harmless fallback.
...
- **No unreviewed UGC is rendered** — sender and recipient local models perform
  independent safety passes, with deterministic PII/link checks and harmless
  failure fallbacks.
