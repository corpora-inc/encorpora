# Corpan City — Release Engineering (0.1.0 → catalog)

**Status:** design / runbook. This is the P3 deliverable from
`docs/PRODUCTION_ROADMAP.md`. It describes how to ship Corpan City **0.1.0** to
...
- **Catalog**: one **`catalog-v3` `packType: "game"` entry**, `zipUrl` =
  `https://encorpora.io/corpan/packs/corpan-city.zip`, mirroring the `world_radio`
...
- `build.lib` → **IIFE**, `name: "CorpanCity"`, single file `dist/app.js`.
...
narration pack (~50–60 MB) — Corpan City is a *code* pack, not a media pack.
...
- **Kill the dev catalog entry**: `corpan-app/src/contentPacks/catalog.ts`
  ...
  `/packs/corpan-city/manifest.json`. That is a *dev-only* convenience. On release
...
Corpan City is a **game pack** → it goes in **`catalog-v3`** as a
...
Corpan City rides; nothing new is needed.**
...
```json
{
  "id": "corpan_city",
  "name": "Corpan City",
  "nameLocalized": { "en": "Corpan City", "es": "...", "...": "(~50 langs)" },
  "version": "0.1.0",
  "manifestUrl": "https://encorpora.io/corpan/packs/corpan-city/manifest.json",
  "zipUrl": "https://encorpora.io/corpan/packs/corpan-city.zip",
...
- **No `languages` field**: Corpan City is language-agnostic (the Quest picks the
...
- This is the *only* in-binary Corpan City metadata; the catalog overrides it OTA.
...
**Decision: Corpan City 0.1.0 ships FREE (public ZIP, `purchase.type: "free"`).**
...
- The two-ZIP preview/full model (`installManager.ts::isTwoZipEntry`, signed
  ...
  **media** packs where the paid bytes (audio) are the product and a server-side
  truncation makes sense (reader narrations). Corpan City's value is the *code +
...
- The right monetization for Corpan City, when it comes, is **Plus-gated premium
...
**Corpan City gets its OWN art** — never reuse the Corpán or any other pack logo.
...
1. The pack's `[Unreleased]` block in `packs/corpan-city/CHANGELOG.md` is large
...
4. **Cross-unit**: this release does not bump the core app (the host already
   supports the install/mount path). If a future Corpan City feature needs a host
...
- [ ] Pushed to `main` (or workflow_dispatch) → GH Pages publishes:
      `https://encorpora.io/corpan/packs/corpan-city.zip` and
      `.../corpan-city/manifest.json` resolve (200).
...
- [ ] **Embedded in corpan-app**: catalog shows Corpan City with the right
...
5. **Flip to stable** (F): change `channel` to `stable`, re-push catalog,
   invalidate CloudFront. Corpan City now appears for all eligible hosts.
