...
/**
 * buildings.ts — PREMIUM colonial paper-cutout buildings for Corpan City.
 *
    ...
    this.painter
      ...
      .then((bitmap) => {
        ...
        try {
          ...
          console.error("[corpan-city/buildings] façade bitmap upload failed", e)
        }
        bitmap.close?.()
      })
      .catch((e) => {
        if (this.disposed) return
        console.error("[corpan-city/buildings] façade worker paint rejected → main-thread fallback", e)
        ctx.clearRect(0, 0, w, h)
        drawFacade(ctx, w, h, spec)
        tex.update()
      })
