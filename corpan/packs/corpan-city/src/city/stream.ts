...
export function createStreamManager(opts: StreamOptions): StreamManager {
  ...
  const notify = () => {
    ...
    if (sig !== lastSig || draws !== lastDraws) {
      ...
      console.log(
        `[corpan-city/city] near chunks ${enabled.size}/${built.size} built ` +
  ...
  const drain = () => {
    ...
    do {
      ...
      try {
        ...
        console.error(`[corpan-city/city] chunk ${key} build step threw → dropping it (city keeps building)`, e)
        ...
        if (half) {
          ...
          try {
            ...
            console.error(`[corpan-city/city] chunk ${key} half-build dispose threw`, de)
          }
        }
      }
