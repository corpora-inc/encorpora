import { RoomTopology, Scene as WorldSceneSchema, NpcRole, type LearnerPair, type ChallengeContext, type ChallengeToolId, type Quest as QuestT } from "@corpan-city/contracts"
...
import { loadOrMintPlayerId } from "./track/registry"

/**
 * Corpan City — game wiring. Onboarding (pick a safe name, dress an avatar) →
 ...
 * standalone dev). All driven by data validated against @corpan-city/contracts.
 */
...
function loadIdentity(): OnboardingResult | null {
  try {
    ...
    console.warn("[corpan-city] could not read saved identity:", err)
    return null
  }
}
...
function saveIdentity(id: OnboardingResult) {
  try {
    ...
    console.warn("[corpan-city] could not save identity:", err)
  }
}

function bindIdentityToPlayer(identity: OnboardingResult, playerId: string): OnboardingResult {
  if (identity.name.playerId === playerId) return identity
  return {
    ...identity,
    name: {
      ...identity.name,
      playerId: playerId as OnboardingResult["name"]["playerId"],
    },
  }
}
...
export function startGame(container: HTMLElement, host?: unknown): GameHandle {
  ...
  const playerId = loadOrMintPlayerId()
  ...
  if (host && !hostHasCorpus) {
    console.warn(
      "[corpan-city] host has no getRandomEntry — challenges fall back to the built-in mock corpus",
    )
  }
  ...
  const begin = async (identity: OnboardingResult) => {
    ...
    try {
      ...
      console.error("[corpan-city] entry resolve failed; using default pair:", err)
  ...
  if (saved) {
    const identity = bindIdentityToPlayer(saved, playerId)
    if (identity !== saved) saveIdentity(identity)
    void begin(identity)
  } else {
    runOnboarding(container, { playerId, native: nativeLocale() })
      .then((res) => {
        const identity = bindIdentityToPlayer(res, playerId)
        saveIdentity(identity)
        return begin(identity)
      })
      .catch((err) => {
        console.error("[corpan-city] onboarding failed; using defaults:", err)
        return begin(defaultIdentity(playerId))
      })
  }
...
function buildWorld(
  ...
  const loadActiveQuestId = (): string => {
    try {
      ...
      console.warn("[corpan-city] could not read active quest:", err)
    }
    return entryQuestId
  }
  const saveActiveQuestId = (id: string) => {
    try {
      ...
      console.warn("[corpan-city] could not persist active quest:", err)
    }
  }
  let quest: QuestT = getQuest(loadActiveQuestId()) ?? getQuest(entryQuestId)!
  if (!quest) throw new Error("[corpan-city] no quests in catalog")
  ...
  const applyAvatarChange = (next: typeof currentAvatar) => {
    ...
    try {
      ...
      console.error("[corpan-city] redress failed:", e)
    }
  ...
  const riverwalk =
    waterEdgeZ != null
      ...
      : null
  if (!riverwalk) console.warn("[corpan-city] no water edge (layout.water / bridge_n) — riverwalk disabled")
  ...
  let unsubActive: (() => void) | null = activeEngine.subscribe((e) => {
    for (const fn of proxyListeners) {
      try {
        ...
        console.error("[corpan-city] quest proxy subscriber threw:", err)
      }
    }
  })
    ...
    openDialogue = npcRuntime.open({
      ...
      onIntent: (intent) => {
        ...
        runChallenge(intent.tool, ctx, chHost, {
          ...
          .catch((e) => console.error("[corpan-city] challenge failed:", e))
          .finally(() => {
  ...
  const onComplete = async (completed: QuestT) => {
    ...
    try {
      ...
      if (picked) {
        ...
        else console.warn("[corpan-city] picked unknown next quest:", picked.chosenQuestId)
      }
    } catch (e) {
      console.error("[corpan-city] quest interlude failed:", e)
    } finally {
      interludeOpen = false
      setWorldActive(true)
      refreshChrome()
  ...
  const addTransitPortal = (
    ...
    if (!anchor) {
      console.warn(`[corpan-city] no \`${anchorId}\` anchor — ${vignetteId} portal disabled`)
      return
    }
    const enter = async (id: string): Promise<void> => {
      ...
      try {
        ...
        console.error(`[corpan-city] ${vignetteId} vignette failed:`, e)
      } finally {
  ...
  if (typeof window !== "undefined" && isDevBuild) {
    ...
    const fire = (vid: string, anchorId: string) => {
      void vignetteHost
        ...
        .catch((e) => console.error("[corpan-city] dev transit enter failed:", e))
    }
  ...
  // ── PHONE-AS-MENU (corpan-city-phone-os) ────────────────────────────────────
  ...
  // ── The PHONE: the SINGLE in-game menu (corpan-city-phone-os) ───────────────
  ...
  const editIdentity = async (): Promise<void> => {
    ...
    try {
      ...
      console.error("[corpan-city] edit identity failed:", err)
    } finally {
      setWorldActive(true)
  ...
  function relocalize(next: Immersion): void {
    ...
    try {
      ...
      console.error("[corpan-city] immersion relocalize failed:", err)
    }
  }
