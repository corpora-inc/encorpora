 ...
 */
export function wireNpcDictation(
  ...
  let state: "idle" | "starting" | "live" | "stopping" = "idle"
  let destroyed = false

  const setState = (next: typeof state) => {
    state = next
    micBtn.classList.toggle("is-starting", next === "starting")
    micBtn.classList.toggle("is-live", next === "live")
    micBtn.classList.toggle("is-stopping", next === "stopping")
    micBtn.disabled = next === "starting" || next === "stopping"
    micBtn.setAttribute("aria-pressed", String(next === "live"))
    if (next === "starting" || next === "stopping") micBtn.setAttribute("aria-busy", "true")
    else micBtn.removeAttribute("aria-busy")
    if (next === "idle") micBtn.style.removeProperty("--wp-npc-mic-ring")
  }
  ...
  async function start() {
    ...
    session.onLevel((rms) => {
      ...
      const ring = Math.round(4 + Math.max(0, Math.min(1, rms)) * 12)
      micBtn.style.setProperty("--wp-npc-mic-ring", `${ring}px`)
    })
    session.onError((code, message) => {
      session = null
      if (code === "INTERRUPTED" || code === "CANCELLED") { setState("idle"); return }
