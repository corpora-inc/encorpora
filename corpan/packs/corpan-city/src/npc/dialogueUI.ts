 ...
 * logic. The keyboard is always available; when the host exposes ASR, the mic
 * becomes an explicit icon-and-color recording control.
 *
    ...
    <section class="wp-npc-panel" role="dialog" aria-modal="true" aria-label="${escapeAttr(opts.npcName)}">
      ...
      <footer class="wp-npc-composer">
        <button
          ...
          aria-pressed="false"
        >
          <span class="wp-npc-mic-icon" aria-hidden="true">${ICON_MIC}<span class="wp-npc-mic-stop"></span></span>
        </button>
