...
regenerated by `tools/make-icon.py` rather than committed as an opaque blob. It
is written as PNG colour type 2 — no alpha channel, because an app icon that
carries one is rejected by App Store validation (ITMS-90717), and the art pass
comes out of the same script.
