...
// this card. The caller decides that — `borrowAcrossZero.contrastRep` is the
// declaration, and CG-22 checks it — and this module only draws what it is given
// and returns `null` when the board cannot honestly be built.
//
// It is library code, not pack code, for the same reason the arithmetic is: the
// contradiction is a property of the mathematics, and a pack that redrew it its
// own way would be free to draw one that does not hold.

import { toScaled } from "../math/rational.ts";
...
export function countingBoard(exercise: Exercise, submitted: AnswerValue): CountingBoard | null {
  ...
  const places = Array.from({ length: width }, (_unused, i) => width - 1 - i);
