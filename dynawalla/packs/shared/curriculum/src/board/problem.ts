 ...
 * the same surface the mal-rules are required to use (`generators/columnOp/digits.ts`
 * says so in as many words). Nothing here reaches into the generator's private draw.
 *
 * This is NOT the number layer. `NumberFormat` — decimal separator, grouping,
 * numbering system, direction — owns every locale question. What this module
 * produces is the *unformatted* digit string a formatter will later take as input,
 * and the digit array the counting board places counters from. An English build
 * renders `5001`, not `5,001`, until that layer lands.
 *
 * It lives in the library rather than in a pack because every pack that draws a
 * column problem needs exactly this reading of it, and two packs reading the
 * contract two ways is how two packs come to disagree about what a child was
 * asked.
 */

import { toDecimalString, toScaled } from "../math/rational.ts";
import type { Rational } from "../math/rational.ts";
import { PROMPT_KEY_ADD, PROMPT_KEY_SUB, SLOT_BOTTOM, SLOT_TOP } from "../generators/columnOp/constants.ts";
 ...
 * precision — a third is not a decimal, and rendering `0.333` would be a pack
...
export function plainDigits(value: Rational, decimalPlaces: number): string | null {
  return toDecimalString(value, decimalPlaces);
}
...
/** The answer as a plain digit string, or `null` for a schema no slate can write. */
export function writtenAnswer(exercise: Exercise): string | null {
  ...
  const places =
    exercise.schema.kind === "choice" || exercise.schema.kind === "fraction" ? 0 : exercise.schema.decimalPlaces;
