"""Same as sample_lra_test.py but LRA=8 (more compressed, mobile-optimized).

Output:
  voices/data/pre-mastered/<name>__mastered-18-lra8.wav
  voices/data/samples/<voice>-mastered-18-lra8/en.wav
"""
...
def main() -> None:
    ...
    for slug, ref_filename in AUDITION_VOICES:
        ...
        stem = ref.stem
        out = PREMASTER_DIR / f"{stem}__mastered-18-lra8.wav"
        master(ref, out)
    ...
    for slug, ref_path in variants:
        out = SAMPLES_DIR / f"{slug}-mastered-18-lra8" / "en.wav"
        ...
        print(f"  {slug}-mastered-18-lra8  dur={dur_s:5.2f}s gen={gen_s:5.2f}s RTF={rtf:.2f} -> {out}")
