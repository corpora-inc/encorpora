...
if $BUILD_EPUB; then
  pandoc "${SELECTED_FILES[@]}"   --to=epub3   --mathml   --css=epub.css   --toc   --toc-depth=2   --metadata title="$TITLE"   --metadata author="$AUTHOR"   --metadata lang="$LANG"   --metadata date="$DATE"   --metadata publisher="$PUBLISHER"   --metadata isbn="$ISBN"   -o "$OUTPUT_EPUB"
fi
